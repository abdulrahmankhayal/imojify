...
class imoji:
    ...
    def get_img_path(self,emji):
        if emoji.is_emoji(emji):
            try: path = self.uni_code_dirc[emji]
            except: 

                try :path=self.name_dirc[emoji.demojize(emji).lower().replace('-','_')[1:-1]]
                except:
            ...
            if not list_im : return os.path.join(dir, './Images/Empty.png')
